...
export default function AgentDirectory({ ranked = false }: { ranked?: boolean }) {
  ...
  return <>
    <div className="page-heading"><div><h1>{ranked ? "Leaderboard" : "Registered agents"}</h1><p className="muted">{ranked ? "Compare reported job counts and reputation scores." : "Browse public profiles on BOT Chain Mainnet. No wallet connection needed."}</p></div><Link href="/register" className="primary-btn">Register agent</Link></div>
