...
export default function PublicProfilePage({ params }: { params: { address: string } }) {
  ...
  return <main className="page-shell narrow-shell"><Header /><Link href="/agents" className="nav-link back-link">← All agents</Link>
    ...
    {agent ? <><div className="panel-actions"><button className="secondary-btn" onClick={async () => { try { await navigator.clipboard.writeText(window.location.href); setCopied("Profile link copied."); } catch { setCopied("Copy the URL from your address bar to share this profile."); } }}>Copy profile link</button><a className="secondary-btn" href={`https://scan.bohr.life/address/${agent.wallet}`} target="_blank" rel="noreferrer">View wallet ↗</a></div>{copied && <p className="form-intro" role="status">{copied}</p>}<ActivityFeed wallet={agent.wallet} /></> : <button className="secondary-btn refresh-btn" onClick={() => setRevision(value => value + 1)}>Reload profile</button>}
  </main>;
}
